...
# Similar to Distant_metastasis_Hazard_Ratio.R but, this time we inspect Overall Survival
dat <- with(dbsurv, {list(
  OS = as.integer(Status), 
  A = as.integer(APM),
  D = as.integer(histo), #   "UPS","MFS","DDLPS","MLPS","LMS","GIST","CHS","CH"
  G = as.integer(Grade),
  months_to_event = last_fup_m)})

file <- file.path("OS_grade.stan")
mod <- stan_model(file)
...
write_xlsx(coef_alpha, "Values_for_CI_OS.xlsx")

# plot(precis(as.data.frame(med_s_diff)))
# abline(v=1,lty=2)

# Since will graphs violin plots, first we change the column names into
# Original histo type names and save the dataframe into a 'OS_Graded.csv' file

colnames(differences_df) <- c('UPS','MFS','DDLPS','MLPS','LMS','GIST','CHS','CH') 
write.csv(differences_df, "OS_Graded.csv", row.names=FALSE)
med_s_diff <- read.csv("OS_Graded.csv")
...
# }
# dev.off()
pdf('Histogram_of_Graded_OS.png', width = 10, height = 20)
par(mfrow=c(8,1))
for(i in 1:8) {
